...
void mass_center_vertical_plot(unsigned short y, unsigned char *c){
    // assume size 240 char array
    int i;
    int max_value = 0;
    for(i=0; i<240; i++){
        if(c[i]>max_value){
    ...
    for(j=0; j<240; j++){
        for(k=0; k<50; k++){
            if(c[j]==max_value){
                LCD_drawPixel(j, y+k, ILI9341_WHITE);
            }
            else{
                LCD_drawPixel(j, y+k, ILI9341_BLACK);
            }
        }
    }
}
